        ...
        for sop in itertools.product(single_ops, repeat=3):
            ...
            try:
                ...
                if result:
                    ...
                    equations.add((plate_num + '-' + str(last_digit), equation, total_num))
                    total_num += 1
                else:
                    equations.add((plate_num + '-' + str(last_digit), '', 0))
            except:
                pass
...
for i in range(0, 1000):
    for j in range(0, 10):
        ...
        plate_num, solution, _ = all_equations[-1]
        all_equations[-1] = (plate_num, solution, count)

# Write equations to CSV
with open('equations.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["plate_num", "solution", "total_num"])
    writer.writerows(all_equations)

print("Equations have been written to equations.csv!")
